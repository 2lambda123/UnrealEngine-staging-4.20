...
public:
	 ...
	 * Sets all Custom Splash Screen related data 
	 *
	 * @param	UseCustomData		Whether or not to use the Custom Data
	 * @param	CustomTransform		Custom Transform for the Splash Screen
	 * @param	CustomWorldScale	Custom World Scale for the Splash Screen
	 */
	UFUNCTION(BlueprintCallable, Category = "SteamVR")
		static void SetCustomSteamSplashScreenData(bool UseCustomData, FTransform CustomTransform, float CustomWorldScale = 1.0f);

	/**
	 * Jacob Saueressig - Engine Mod
	 * Gets the controller's model
	 */
	UFUNCTION(BlueprintCallable, Category = "SteamVR")
		static FString GetFStringControllerModel();

...
void FSteamVRHMD::UpdateLayer(struct FSteamVRLayer& Layer, uint32 LayerId, bool bIsValid) const
{
	...
	// Adam Rehberg - Engine Mod Start - Vive on SteamVR
	// Added in a custom Splash Screen option so we can manually set one Transform
	//		and all of the future splash screens will use that same location.
	if (CustomSplashScreenData)
	{
		vr::HmdMatrix34_t CustomHmdTransform;
		TransformToSteamSpace(CustomSplashScreenTransform, CustomHmdTransform, CustomSplashScreenWorldScale);
		OVR_VERIFY(VROverlay->SetOverlayTransformAbsolute(Layer.OverlayHandle, VRCompositor->GetTrackingSpace(), &CustomHmdTransform));
		return;
	}
	// Adam Rehberg - Engine Mod End

...
bool FOnlineVoiceOculus::RegisterRemoteTalker(const FUniqueNetId& UniqueId)
{
	...
	UE_LOG(LogVoice, Verbose, TEXT("RegisterRemoteTalker ovr_Voip_Start called with OculusID: %s"), *(OculusId.ToString()));
	auto OculusPeerID = new FUniqueNetIdOculus(OculusId.GetID());
	UE_LOG(LogVoice, Verbose, TEXT("Adding %llu to remote talker list - Engine Mod - 1"), OculusId.GetID());
...
bool FOnlineVoiceOculus::UnregisterRemoteTalker(const FUniqueNetId& UniqueId)
{
	...
	UE_LOG(LogVoice, Verbose, TEXT("UnregisterRemoteTalker ovr_Voip_Stop called with OculusID: %s"), *(OculusId.ToString()));
	return true;
...
void FOnlineVoiceOculus::RemoveAllRemoteTalkers()
{
	...
	{
		//// Engine Mod - Adam Rehberg - Added if IsValid check
		if (!(RemoteTalker.TalkerId.IsValid()))
			continue;
...
void FOnlineVoiceOculus::ProcessRemoteVoicePackets()
{
	...
	for (auto &RemoteTalker : RemoteTalkers)
	{
		...
		//// Engine Mod - Adam Rehberg - Added if IsValid check
		if (!RemoteTalkerId.IsValid())
			continue;

		auto BufferSize = ovr_Voip_GetOutputBufferMaxSize();

		//// Engine Mod - Adam Rehberg - Added Buffer Size check
		if (BufferSize <= 0)
			continue;
...
int32 FOnlineVoiceOculus::IndexOfRemoteTalker(const FUniqueNetId& TalkerId)
{
	...
	for (int32 i = 0; i < RemoteTalkers.Num(); ++i)
	{
		auto const &RemoteTalker = RemoteTalkers[i];
		if (!(RemoteTalker.TalkerId.IsValid()))
			continue;

		if (*RemoteTalker.TalkerId == TalkerId)
		{
			Index = i;
			break;
		}
...
void FOnlineVoiceOculus::OnVoipConnectionRequest(ovrMessageHandle Message, bool bIsError) //const //// Adam Rehberg Engine Mod Start/Stop - Commented out the const
{
	...
	UE_LOG(LogVoice, Verbose, TEXT("OnVoipConnectionRequest ovr_Voip_Accept called with PeerID: %d"), PeerID);
	auto OculusPeerID = new FUniqueNetIdOculus(PeerID);
	UE_LOG(LogVoice, Verbose, TEXT("Adding %llu to remote talker list - Engine Mod - 2"), PeerID);
	auto t = RemoteTalkers.AddZeroed();
	int32 AddIndex = RemoteTalkers.AddZeroed();
	auto RemoteTalker = &RemoteTalkers[AddIndex];
	RemoteTalker->TalkerId = MakeShareable(OculusPeerID);
...
void FOnlineVoiceOculus::OnVoipStateChange(ovrMessageHandle Message, bool bIsError)
{
	...
	//// Engine Mod - Adam Rehberg - Added if IsValid check
	if (OculusPeerID->IsValid())
	{
		auto NetIDFromPeerID = *OculusPeerID;
		if (NetIDFromPeerID.IsValid())
		{
			auto Index = IndexOfRemoteTalker(NetIDFromPeerID);
			if (Index == INDEX_NONE)
			{
				if (State == ovrPeerState_Connected)
				{

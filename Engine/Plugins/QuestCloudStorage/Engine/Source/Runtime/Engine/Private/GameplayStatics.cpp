...
bool UGameplayStatics::SaveGameToSlotAtPath(USaveGame* SaveGameObject, const FString& SlotName, const FString& Path, const int32 UserIndex)
{
	ISaveGameSystem* SaveSystem = IPlatformFeaturesModule::Get().GetSaveGameSystem();
	// If we have a system and an object to save and a save name...
	if (SaveSystem && SaveGameObject && (SlotName.Len() > 0))
	{
		...
		// Stuff that data into the save system with the desired file name and path
		return SaveSystem->SaveGameAtPath(false, *SlotName, *Path, UserIndex, ObjectBytes);
	}
	return false;
}

bool UGameplayStatics::DoesSaveGameExistAtPath(const FString& SlotName, const FString& Path, const int32 UserIndex)
{
	if (ISaveGameSystem* SaveSystem = IPlatformFeaturesModule::Get().GetSaveGameSystem())
	{
		return SaveSystem->DoesSaveGameExistAtPath(*SlotName, *Path, UserIndex);
	}
	return false;
}

USaveGame* UGameplayStatics::LoadGameFromSlotAtPath(const FString& SlotName, const FString& Path, const int32 UserIndex)
{
	USaveGame* OutSaveGameObject = NULL;
	...
	if (SaveSystem && (SlotName.Len() > 0))
	{
		// Load raw data from slot
		TArray<uint8> ObjectBytes;
		bool bSuccess = SaveSystem->LoadGameFromPath(false, *SlotName, *Path, UserIndex, ObjectBytes);
		if (bSuccess)
		{
			FMemoryReader MemoryReader(ObjectBytes, true);
...
bool UGameplayStatics::DeleteGameInSlotAtPath(const FString& SlotName, const FString& Path, const int32 UserIndex)
{
	if (ISaveGameSystem* SaveSystem = IPlatformFeaturesModule::Get().GetSaveGameSystem())
	{
		return SaveSystem->DeleteGameAtPath(false, *SlotName, *Path, UserIndex);
	}
	return false;
}
